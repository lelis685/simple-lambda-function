...
import com.lambda.aws.apis.dto.Order;
...
public class ReadOrdersLambda {
	...
	public APIGatewayProxyResponseEvent getOrders(APIGatewayProxyRequestEvent request) throws JsonMappingException, JsonProcessingException {
		...
		ScanResult scanResult = dynamoDB.scan(new ScanRequest(System.getenv("ORDERS_TABLE")));
		
		List<Order> orders = scanResult.getItems().stream().map(item -> new Order(
